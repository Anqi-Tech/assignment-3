...
export class Orchestrator {
    constructor() {
        this.name = "casual_sad_anger_synthesis";
        this.agentByName = {
    ...
    async _planSegments(contents) {
        ...
        Each item has: emotion in {casual, sad, anger} and a short 'cue' that hints what that sentence should convey.
        ...
        try {
            ...
            const segments = Array.isArray(parsed?.segments)
                ...
                : [];
            const reasons =
                typeof parsed?.reasons === "string" ? parsed.reasons : "";
            const sanitized = segments
                .map((s) => ({
                    emotion: String(s.emotion || "").toLowerCase(),
                    cue: String(s.cue || "").trim(),
                }))
                .filter(
                    (s) =>
                        ["casual", "sad", "anger"].includes(s.emotion) && s.cue
                );
            ...
            return {
                segments: [
                    {
                        emotion: "sad",
                        cue: "Validate feelings and offer a gentle step.",
                    },
                ],
                reasons: "Defaulted",
            };
        ...
        for (const seg of plan) {
            ...
            const extendedContents = [
                ...contents,
                {
                    ...
                    parts: [
                        {
                            text: `Instruction: Write ONE sentence for a composite reply. Focus: ${seg.cue}`,
                        },
                    ],
                },
            ];
            const sentence =
                (await this._respondWith(agentName, extendedContents)) || "";
    ...
    _toTaggedText(sentences) {
        return sentences.map((s) => `(${s.emotion})[${s.text}]`).join(" ");
    }
    ...
    async orchestrate(contents) {
        ...
        const frameSet = {
            frames: {
                persona: {
                    value: "multi",
                    rationale: [reasons || "multi-emotion synthesis"],
                },
            },
        };
